...
class WebpackCdnPlugin {
  ...
  apply(compiler) {
    ...
    compiler.hooks.compilation.tap('WebpackCdnPlugin', (compilation) => {
      WebpackCdnPlugin._getHtmlHook(
        compilation,
        'beforeAssetTagGeneration',
        'htmlWebpackPluginBeforeHtmlGeneration'
      ).tapAsync('WebpackCdnPlugin', (data, callback) => {
        ...
        if (moduleId !== false) {
          ...
          if (modules) {
            ...
            data.assets.js = WebpackCdnPlugin._getJs(
              modules,
              ...getArgs
            ).concat(data.assets.js)
            data.assets.css = WebpackCdnPlugin._getCss(
              modules,
              ...getArgs
        ...
        compiler.hooks.thisCompilation.tap('WebpackCdnPlugin', () => {
          compiler.hooks.compilation.tap(
            ...
            (compilation) => {
              ...
              ).tapPromise('WebpackCdnPlugin', this.alterAssetTags.bind(this))
            }
          )
        })
      })
  ...
  async alterAssetTags(pluginArgs) {
    ...
    const filterTag = (tag) => {
      const url =
        (tag.tagName === 'script' && tag.attributes.src) ||
        (tag.tagName === 'link' && tag.attributes.href)
      return (
        url &&
        prefixes.filter((prefix) => url.indexOf(prefix) === 0).length !== 0
      )
    }
    ...
    if (pluginArgs.assetTags) {
      await Promise.all(
        pluginArgs.assetTags.scripts.filter(filterTag).map(processTag)
      )
      await Promise.all(
        pluginArgs.assetTags.styles.filter(filterTag).map(processTag)
      )
    } else {
  ...
  static _cleanModules(modules, pathToNodeModules) {
    modules.forEach((p) => {
      p.version = WebpackCdnPlugin.getVersionInNodeModules(
        p.name,
        pathToNodeModules
      )
    ...
    modules
      ...
      .forEach((p) => {
        ...
        p[pathsKey].forEach((s) =>
          files.push(
            prefix +
              (moduleSpecificUrl || url).replace(paramsRegex, (m, p1) => {
...
/**
 * ,"httpStatus":"200",
 * "ua":"Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.106 Electron/4.0.6 Safari/537.36",
 * "device":"27147081255147376",
 * "eventTime":"",
 * "apiTime":"1606438056130",
